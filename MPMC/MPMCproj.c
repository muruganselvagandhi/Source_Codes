...
sbit LED_Light = P2^3;  // LED controlled by LDR 
...
void readDHT11Data();           // Function to read from DHT11
...
int receiveDHT11Byte() {
    int i, result = 0;
    for (i = 0; i < 8; i++) {
        while (DHT11 == 0);           // Wait for start
        Delay(30);                    // Delay to check value
        if (DHT11 == 1) {             // If high
            result = (result << 1) | 1;
        } else {                      // If ow
            result = (result << 1);
        }
        while (DHT11 == 1);           // Wait for end of 
    }
    return result;
}
